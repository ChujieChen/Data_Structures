...
//    	System.out.println("Inserting "+x);
    ...
    void erase(int x) {
    	...
    	// TODO
    	/**
    	 * not working since find(Vertex, int) would not give us NextN in fact
    	// Lecture Video 5:15
    	// When N is the largest, no NextN would be found
//    	System.out.println("Erasing "+x);
    	VertexPair leftRight = find(root, x);
		Vertex N = leftRight.right;
		System.out.println("N = "+N.key);
		Vertex NextN = leftRight.left;
		System.out.println("NextN = "+NextN.key);
		root = splay(NextN);
		root = splay(N);
		Vertex L = root.left;
		Vertex R = root.right;
		...
		update(root);
		*/
    	// inspired by discussions: use split and merge instead
